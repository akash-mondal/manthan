...
async def test_chat_retries_on_malformed_200_and_raises_after_exhaustion(
    ...
    Gemini occasionally returns HTTP 200 with a semantic error
    ...
    transport = httpx.MockTransport(handler)
    # Gemini cascade: primary (gemini-3.1-pro-preview) + 1 fallback
    # (gemini-3-flash-preview) = 2 models, each with max_retries=2
