...
def narrative_delta(text: str) -> AgentEvent:
    ...
    return AgentEvent(
        ...
        data={"text": _mask(text)},
    )


def narrative_done() -> AgentEvent:
    """Marks the end of a streamed narrative block.

    Emitted after the last ``narrative_delta`` for a given assistant
    message. The frontend uses this to (a) extract ``---NEXT---``
    follow-up chips from the accumulated text, and (b) lock the block
    so subsequent deltas open a new block instead of continuing it.
    """
    return AgentEvent(type="narrative_done", data={})
