...
async def _stream_description(
    ...
    try:
        ...
        api_key = (
            ...
            or get_settings().gemini_api_key.get_secret_value()
        )
        if not api_key:
        ...
        try:
            async with (
                ...
                # Gemini's OpenAI-compat endpoint forwards the standard
                # OpenAI SSE stream: lines of ``data: {json}`` separated
                # by blank lines, terminated by ``data: [DONE]``. Each
                # delta carries ``choices[0].delta.content`` (primary
